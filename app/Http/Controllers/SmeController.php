...
class SmeController extends Controller
{
    public function get_token()
    {
        while (1) {
            ...
            $inserted = DB::connection('mysql2')->table('custom_tokens')->insert(
                [
    ...
    public function get_table_structure(Request $request)
    {
        ...
        $token = DB::connection('mysql2')->table('custom_tokens')->where('token', $request->token)->first();
        ...
        if ($difference_in_seconds < 10) {
            $companyinfo = DB::connection('mysql2')->select('DESCRIBE companyinfo');
            $company_applicant = DB::connection('mysql2')->select('DESCRIBE company_applicant');
            $company_attachments = DB::connection('mysql2')->select('DESCRIBE company_attachments');
            $company_owner_infos = DB::connection('mysql2')->select('DESCRIBE company_owner_infos');
            return response()->json([
                'status' => 'success',
            ...
            $token = Str::random(30);
            DB::connection('mysql2')->table('custom_tokens')->where('token', $request->token)->delete();
            while (1) {
                $characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
                $charactersLength = strlen($characters);
                $randomString = '';
    ...
    public function get_ubid(Request $request)
    {
        ...
        if ($difference_in_seconds < 10) {
            ...
            $application_id_DB = DB::connection('mysql2')->table('companyinfo')->latest('application_id')->first();
            ...
            try {
                ...
                $application_create = DB::connection('mysql2')->table('companyinfo')->insert($data);
