...
class NiscController extends Controller
{
    ...
    public function get_token()
    {
        while (1) {
            ...
            $inserted = DB::table('custom_tokens')->insert(
                [
    ...
    public function get_table_structure(Request $request)
    {
        ...
        $token = DB::table('custom_tokens')->where('token', $request->token)->first();
        ...
        if ($difference_in_seconds < 10) {
            ...
            $token = Str::random(30);
            DB::table('custom_tokens')->where('token', $request->token)->delete();
            while (1) {
                $characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
                $charactersLength = strlen($characters);
                $randomString = '';
